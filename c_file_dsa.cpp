...
int main()
{
    ...
    struct Hospital e ; 
    struct Hospdetails* nearby = (struct Hospdetails*)calloc(10,sizeof(struct Hospdetails));
    ...
    short j ; 
     ...
     for(j = 0 ; j < i ; j++)
     {
      ...
      if (i == 0) {
        printf("No hospitals found near your location.\n");
        fclose(fptr);
        return 1;}
    ...
    fseek(shivajinagar, 0, SEEK_END);
    long size = ftell(shivajinagar);
    short recordCount = size / sizeof(struct Location);
    rewind(shivajinagar);
	...
	short useridx = locate(lat,lon);
		if (useridx == -1) {
    printf("User location is not present in the map nodes.\n");
    return 1;}
	...
	for(i=3; i < useful ; i++)
	{
		char *temp = concat("OutputFile ", concat(finalHospitals[i].hospital.name, ".txt"));
        remove(temp);
        free(temp);
	...
	printf("BEST_HOSPITAL=%s\n", finalHospitals[0].hospital.name);
  printf("ROUTE_FILE=OutputFile %s.txt\n", finalHospitals[0].hospital.name);
  printf("\nHospital: %s\nDistance: %.2f m\nETA: %.2f mins\n", nearby[i].hospital.name, nearby[i].dis, nearby[i].time);
